...
a_low = 0
a_high = [knob[1]['max_value'] for knob in list(knob_config.items())[:]]
a_high = max(a_high)
    ...
    def act(self, cur_state):
        ...
        for i in range(action[0].shape[0]):
            if action[0][i] <= self.env.default_action[i]:
                print("[Action %d] Lower than DEFAULT: %f" % (i, action[0][i]))
                action[0][i] = self.env.default_action[i]
                # return self.env.action_space.sample()
            elif action[0][i] > self.env.a_high[i]:
                print("[Action %d] Higher than MAX: %f" % (i, action[0][i]))
